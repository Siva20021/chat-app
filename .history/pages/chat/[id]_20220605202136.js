...
const Bottombar=(e)=>{
    ...
    const sendMessage=async(e)=>{
        e.preventDefault();
        await addDoc(collection(db,`chats/${id}/messages`),{text:input,sender:user.email,timestamp:serverTimestamp
        })
    ...
    return(
        <FormControl
            p={3}
        >
            <Input placeholder='Type a message...' autoComplete='off' onChange={()=>setInput(e.target.value)}/>
